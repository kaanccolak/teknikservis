...
export default function BayiKargoFisi() {
  const params = useParams()
  const router = useRouter()
  const [bayi, setBayi] = useState<BayiApiRow | null>(null)
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const [bayiRes, shopRes] = await Promise.all([
          fetch(`/api/bayiler/${params.id}`),
          fetch('/api/shop'),
        ])
        const data = (await bayiRes.json()) as {
          bayi?: BayiApiRow | null;
        };
        const shopData = await shopRes.json();
        setBayi(data.bayi ?? null);
  ...
  if (!bayi) return <div style={{ padding: 20 }}>Bayi bulunamadı.</div>
      ...
      <div className="page-bg" style={{ minHeight: '100vh', backgroundColor: '#f3f4f6', padding: '20px' }}>
        ...
        <div className="fis-wrapper" style={{ width: '280px', margin: '0 auto', backgroundColor: 'white', padding: '16px', boxShadow: '0 2px 8px rgba(0,0,0,0.15)', borderRadius: '4px', fontSize: '12px' }}>
          ...
          <div style={{ border: '1px solid #000', padding: '8px', marginBottom: '8px' }}>
            ...
            {bayi.firmaAdi && <div><strong>Firma Adı:</strong> {bayi.firmaAdi}</div>}
            {bayi.yetkiliKisi && <div><strong>Yetkili Kişi:</strong> {bayi.yetkiliKisi}</div>}
            {bayi.phone && <div><strong>Telefon:</strong> {bayi.phone}</div>}
            {bayi.address && <div><strong>Adres:</strong> {bayi.address}</div>}
          </div>

          {(bayi.cargoInfo || bayi.cargoCode) && (
            <div style={{ border: '1px solid #000', padding: '8px', marginBottom: '8px' }}>
              <div style={{ fontWeight: 'bold', marginBottom: '4px' }}>Kargo Bilgileri</div>
              {bayi.cargoInfo && <div><strong>Anlaşmalı Kargo:</strong> {bayi.cargoInfo}</div>}
              {bayi.cargoCode && <div><strong>Anlaşma Kodu:</strong> {bayi.cargoCode}</div>}
            </div>
          )}
